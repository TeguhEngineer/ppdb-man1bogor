    ...
    <div class="bg-white overflow-hidden shadow-sm rounded-xl border border-gray-100 mb-6">
        <div class="p-6 md:p-8 text-gray-900">
            ...
            <div class="mb-6 flex flex-col md:flex-row justify-between items-end gap-4">
                ...
                <!-- Search & Filter -->
                <form method="GET" action="{{ route('admin.verifikasi.index') }}" class="flex flex-col md:flex-row gap-2 w-full md:w-auto">
                    <div class="relative">
                    ...
                    <select name="status" class="rounded-lg border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 text-sm" onchange="this.form.submit()">
                    ...
                    <button type="submit" class="bg-gray-800 hover:bg-gray-900 text-white font-medium py-2 px-4 rounded-lg transition-colors text-sm">
                        Filter
                    </button>
                </form>
            </div>
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="p-4 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <span class="inline-flex items-center p-1.5 rounded-md {{ $pendaftaran->isBiodataLengkap() ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}" title="{{ $pendaftaran->isBiodataLengkap() ? 'Biodata Lengkap' : 'Biodata Belum Lengkap' }}">
                                        <i class="fi fi-rs-document"></i>
                                    </span>
                                    <span class="inline-flex items-center p-1.5 rounded-md {{ $pendaftaran->isBerkasLengkap() ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}" title="{{ $pendaftaran->isBerkasLengkap() ? 'Berkas Lengkap' : 'Berkas Belum Lengkap' }}">
