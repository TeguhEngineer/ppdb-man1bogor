...
class BiodataController extends Controller
{
    ...
    private function saveDataPribadi(Request $request, Pendaftaran $pendaftaran): void
    {
        ...
        $validated = $this->validateBiodata($request, [
            ...
            'kampus' => ['required', 'string', 'max:11'],
            'nama_lengkap' => ['required', 'string', 'max:30'],
            'tempat_lahir' => ['required', 'string', 'max:12'],
            ...
            'berat_badan' => ['required', 'integer', 'min:1', 'max:99'],
            'status_dalam_keluarga' => ['required', 'string', 'max:12'],
            'tinggal_bersama' => ['required', 'string', 'max:9'],
            'anak_ke' => ['required', 'integer', 'min:1', 'max:9'],
            'jumlah_saudara' => ['required', 'integer', 'min:0', 'max:9'],
            'agama' => ['required', 'string', 'max:6'],
            'no_whatsapp' => ['bail', 'required', 'string', 'max:12'],
            'alamat' => ['required', 'string', 'max:75'],
            'desa' => ['required', 'string', 'max:12'],
            'kecamatan' => ['required', 'string', 'max:12'],
            'kabupaten' => ['required', 'string', 'max:15'],
            'provinsi' => ['required', 'string', 'max:10'],
            'kode_pos' => ['required', 'digits:5'],
            'jarak_ke_sekolah' => ['required', 'string', 'max:3'],
            'waktu_tempuh_ke_sekolah' => ['required', 'string', 'max:2'],
            'asal_satuan_pendidikan' => ['required', 'in:SMP,MTS'],
            'nama_asal_sekolah' => ['required', 'string', 'max:25'],
    ...
    private function saveDataOrangtua(Request $request, Pendaftaran $pendaftaran): void
    {
        $validated = $this->validateBiodata($request, [
            ...
            'tempat_lahir_ayah' => ['nullable', 'string', 'max:12'],
            'tanggal_lahir_ayah' => ['nullable', 'date'],
            'pendidikan_terakhir_ayah' => ['required', 'string', 'max:25'],
            'pekerjaan_ayah' => ['required', 'string', 'max:15'],
            'penghasilan_ayah' => ['required', 'string', 'max:10'],
            'no_hp_ayah' => ['required', 'string', 'max:12'],
            'nama_ibu' => ['required', 'string', 'max:20'],
            'nik_ibu' => ['nullable', 'numeric', 'digits:16'],
            'tempat_lahir_ibu' => ['nullable', 'string', 'max:12'],
            'tanggal_lahir_ibu' => ['nullable', 'date'],
            'pendidikan_terakhir_ibu' => ['required', 'string', 'max:25'],
            'pekerjaan_ibu' => ['required', 'string', 'max:15'],
            'penghasilan_ibu' => ['required', 'string', 'max:10'],
            'no_hp_ibu' => ['required', 'string', 'max:12'],
            'nama_wali' => ['nullable', 'string', 'max:150'],
            'nik_wali' => ['nullable', 'numeric', 'digits:16'],
            'tempat_lahir_wali' => ['nullable', 'string', 'max:12'],
            'tanggal_lahir_wali' => ['nullable', 'date'],
            'pendidikan_terakhir_wali' => ['nullable', 'string', 'max:25'],
            'pekerjaan_wali' => ['nullable', 'string', 'max:15'],
            'penghasilan_wali' => ['nullable', 'string', 'max:10'],
            'no_hp_wali' => ['nullable', 'string', 'max:12'],
        ]);
    ...
    private function validationMessages(): array
    {
        return [
            ...
            'integer' => ':attribute harus berupa angka tanpa desimal.',
            'date' => ':attribute harus berupa tanggal yang valid.',
            'in' => ':attribute tidak valid.',
            'exists' => ':attribute tidak valid.',
            'unique' => ':attribute sudah terdaftar.',
            'string' => ':attribute harus berupa teks.',
            'max' => ':attribute maksimal :max karakter/nilai.',
            'no_whatsapp.max' => 'Nomor WhatsApp maksimal 12 karakter.',
            'tinggi_badan.min' => 'Tinggi badan terlalu pendek, mohon cek kembali inputan Anda (Minimal :min cm).',
            'tinggi_badan.max' => 'Tinggi badan tidak wajar, maksimal :max cm.',
            'berat_badan.min' => 'Berat badan terlalu ringan, mohon cek kembali inputan Anda (Minimal :min kg).',
            'berat_badan.max' => 'Berat badan tidak wajar, maksimal :max kg.',
            'anak_ke.min' => 'Anak ke minimal adalah 1.',
            'anak_ke.max' => 'Inputan anak ke tidak wajar.',
            'jumlah_saudara.min' => 'Jumlah saudara minimal 0 (jika anak tunggal).',
            'jumlah_saudara.max' => 'Inputan jumlah saudara tidak wajar.',
        ];
    }
